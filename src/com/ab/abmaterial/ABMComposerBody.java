...
public class ABMComposerBody {
	...
	public ABMComposerBody(ABMPage page) {
		Page=page;
	}
	...
	protected String Build() {
		...
		s.append("<div id=\"" + GroupID.toLowerCase() + "-bdy\" class=\"abmcpr-bdy " + GroupID.toLowerCase() + "\">");
			...
			for (int i=0;i<Parts.size();i++) {
				if (Parts.get(i).Type==0) {
					if (!Parts.get(i).IsBuild) {
						if (Counter==0) {
							ABMaterial.AddHTML(Page, GroupID.toLowerCase() + "-bdy", "<div id=\"" + Parts.get(i).TextID + "\" class=\"abmcpr-txt\">" + BuildBodyText(Parts.get(i).TextValue) + "</div>");
					...
					if (comp.IsBuild) {
						...
						if (Counter==0) {
							ABMaterial.AddHTML(Page, GroupID.toLowerCase() + "-bdy", comp.Build());
							prevID = comp.RootID();
						} else {
